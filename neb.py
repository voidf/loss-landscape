...
if __name__ == '__main__':
    ...
    net_container = ModelWrapper(DataModel(
        CompareModel(net, CrossEntropyLoss()),
        {
            'train': trainloader,
            'test': testloader
            # 'train': torchvision.datasets.CIFAR10(root=get_relative_path('cifar10/data'), train=True,
            #                                     download=True, transform=tf),
            # 'test': torchvision.datasets.CIFAR10(root=get_relative_path('cifar10/data'), train=False,
    ...
    op = SGD(neb.parameters(), lr=0.1, momentum=0.9)
    neb.path_coords.requires_grad_(False)    
    
    for _ in pbar(range(300), "NEB"):
        neb.apply(True)
        op.step()

    torch.save(neb, 'neb_path.pkl')
